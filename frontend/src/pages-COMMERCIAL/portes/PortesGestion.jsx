...
export default function PortesGestion() {
  ...
  // Navigation items pour le bottom bar
  ...
  if (loading || commercialLoading || immeubleLoading) {
    return (
    ...
    <div className={`flex flex-col h-screen ${base.bg.card} overflow-hidden`}>
      ...
      >
        ...
        <div className="mb-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1">
              <h1
                className={`text-2xl font-bold ${base.text.primary} mb-2 flex items-center gap-2`}
              >
                <MapPin className="h-6 w-6 flex-shrink-0" />
                {immeuble?.adresse || 'Chargement...'}
              </h1>
              <div className="flex flex-wrap items-center gap-4">
                <p className={`text-sm ${base.text.muted} flex items-center gap-1.5`}>
                  <Home className="h-4 w-4" />
                  <span className="font-medium">
                    {stats.total} porte{stats.total > 1 ? 's' : ''}
                  </span>
                </p>
                {immeuble?.createdAt && (
                  <p className={`text-sm ${base.text.muted} flex items-center gap-1.5`}>
                    <CalendarDays className="h-4 w-4" />
                    <span>Créé le {new Date(immeuble.createdAt).toLocaleDateString('fr-FR')}</span>
                  </p>
                )}
              </div>
            </div>
                    ...
                    >
                      <CardContent className="p-5">
                        <div className="space-y-4">
                          ...
                          {porte.rdvDate && (
                            <div
                              className={`${colors.primary.bgLight} border ${colors.primary.border} rounded-lg p-3`}
                            >
                              <div
                          ...
                          {porte.commentaire && (
                            ...
                            >
                              <p className={`text-sm ${base.text.secondary}`}>
                                {porte.commentaire}
                              </p>
                            </div>
                          )}
        ...
        <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
          <DialogContent
            className={`w-[95vw] max-w-md mx-auto ${base.bg.card} ${base.border.light} max-h-[90vh] overflow-y-auto`}
          >
            <DialogHeader className="pb-4">
              <DialogTitle className={`text-lg font-bold ${base.text.primary}`}>
                {selectedPorte?.nomPersonnalise || `Porte ${selectedPorte?.numero}`}
              </DialogTitle>
              <DialogDescription className={`text-sm ${base.text.muted}`}>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <Input
                    placeholder={`Porte ${selectedPorte?.numero} (par défaut)`}
                    value={editForm.nomPersonnalise}
                    onChange={e =>
                      setEditForm(prev => ({ ...prev, nomPersonnalise: e.target.value }))
                    }
                    className={`h-12 text-base ${base.bg.input} ${base.border.input} ${base.text.primary}`}
                  />
                  <p className={`text-xs ${base.text.muted}`}>
                    Ex: "Porte à droite", "Porte gauche", "Appartement A", etc.
                  </p>
                  {editForm.nomPersonnalise && (
                    <Button
                      ...
                      className={`text-xs ${base.text.muted} hover:${base.text.primary}`}
                    >
              ...
              {/* Statut */}
              <div className="space-y-3">
                <label className={`text-sm font-semibold ${base.text.primary}`}>Statut</label>
                <Select
                  value={editForm.statut}
                  onValueChange={value => setEditForm(prev => ({ ...prev, statut: value }))}
                >
                  <SelectTrigger
                    className={`h-12 ${base.bg.input} ${base.border.input} ${base.text.primary} text-base`}
                  >
                    <SelectValue />
                  </SelectTrigger>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 text-base ${base.bg.input} ${base.border.input} ${base.text.primary} rounded-lg border focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none transition-all duration-200`}
              ...
              {/* Commentaire */}
              <div className="space-y-3">
                <label className={`text-sm font-semibold ${base.text.primary}`}>Commentaire</label>
                <Textarea
                  placeholder="Ajouter un commentaire..."
                  value={editForm.commentaire}
                  onChange={e => setEditForm(prev => ({ ...prev, commentaire: e.target.value }))}
                  rows={4}
                  className={`text-base ${getInputClasses()}`}
                />
              </div>
              ...
              {selectedPorte?.nbRepassages > 0 && (
                <div
                  className={`p-4 ${colors.warning.bgLight} rounded-lg border ${colors.warning.border}`}
                >
                  <div className={`flex items-center space-x-3 ${colors.warning.text}`}>
                    <RotateCcw className="h-5 w-5 flex-shrink-0" />
                    <div>
                      <p className="text-sm font-semibold">
                        {selectedPorte.nbRepassages} repassage
                        {selectedPorte.nbRepassages > 1 ? 's' : ''} effectué
            ...
            <DialogFooter className="pt-6 pb-2">
              <div className="flex flex-col sm:flex-row gap-3 w-full">
                <Button
                  ...
                  className={`flex-1 h-12 text-base ${getButtonClasses('outline')}`}
                >
                  Annuler
                </Button>
                <Button
                  ...
                  disabled={isUpdating}
                  className={`flex-1 h-12 text-base ${getButtonClasses('primary')}`}
                >
